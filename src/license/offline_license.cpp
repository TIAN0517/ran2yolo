...
static bool GetMachineIdHash(char* out, size_t size) {
    ...
    // CPU ID - 完全複製 license_tool.cpp 的內聯彙編方式
    uint32_t edx = 0, ecx = 0;
#if defined(_WIN64)
    // x64: 使用 __cpuid intrinsic（__cpuid 會自動將結果寫入 int[4]）
    int cpuInfo[4] = {0};
    __cpuid(cpuInfo, 1);
    edx = (uint32_t)cpuInfo[3];
    ecx = (uint32_t)cpuInfo[2];
#else
    // x86: 使用內聯彙編（cpuid 結果直接在暫存器中）
    __asm {
    ...
    char cpuId[32] = {0};
    sprintf_s(cpuId, "%08X%08X", edx, ecx);

    // MAC Address
    char macAddr[32] = "000000000000";
    ...
    // Disk Serial
    char diskSerial[32] = "00000000";
    GetVolumeInformationA("C:\\", diskSerial, sizeof(diskSerial), NULL, NULL, NULL, NULL, 0);

    // 與 license_tool.cpp 完全一致
    char combined[256] = {0};
    sprintf_s(combined, "JyTrainer_v1|%s|%s|%s", cpuId, macAddr, diskSerial);
    ...
    if (CryptAcquireContext(&hProv, NULL, NULL, PROV_RSA_FULL, CRYPT_VERIFYCONTEXT)) {
        if (CryptCreateHash(hProv, CALG_MD5, 0, 0, &hHash)) {
            ...
            if (CryptGetHashParam(hHash, HP_HASHVAL, hashBytes, &hashLen, 0)) {
                for (int i = 0; i < 16; i++)
                    sprintf_s(hash1 + i * 2, 3, "%02x", hashBytes[i]);
            }
    ...
    // MD5 #2
    char combined2[64] = {0};
    sprintf_s(combined2, "%s_verify", hash1);
    char hash2[33] = {0};
    if (CryptAcquireContext(&hProv, NULL, NULL, PROV_RSA_FULL, CRYPT_VERIFYCONTEXT)) {
        if (CryptCreateHash(hProv, CALG_MD5, 0, 0, &hHash)) {
            CryptHashData(hHash, (BYTE*)combined2, (DWORD)strlen(combined2), 0);
            BYTE hashBytes[16];
            DWORD hashLen = 16;
            if (CryptGetHashParam(hHash, HP_HASHVAL, hashBytes, &hashLen, 0)) {
                for (int i = 0; i < 16; i++)
                    sprintf_s(hash2 + i * 2, 3, "%02x", hashBytes[i]);
            }
            CryptDestroyHash(hHash);
        }
        CryptReleaseContext(hProv, 0);
    ...
    // 結果 = hash1[0:12] + hash2[0:12] = 24 字元
    strncpy_s(out, size, hash1, 12);
    strncat_s(out, size, hash2, 12);
    return true;
}
...
// ============================================================
// 離線一卡一機驗證 (支持 JYLIC1 和 JYLIC2 格式)
// JYLIC1: RSA 簽名驗證 (需要 license_public.blob)
// JYLIC2: 簡單 MD5 校驗
// ============================================================

static std::string MD5Simple(const char* input, int len) {
    HCRYPTPROV hProv = 0;
    HCRYPTHASH hHash = 0;
    char result[33] = {0};
    if (CryptAcquireContext(&hProv, NULL, NULL, PROV_RSA_FULL, CRYPT_VERIFYCONTEXT)) {
        if (CryptCreateHash(hProv, CALG_MD5, 0, 0, &hHash)) {
            CryptHashData(hHash, (BYTE*)input, len, 0);
            BYTE hash[16];
            DWORD hashLen = 16;
...
// 獲取默認公鑰路徑
static bool GetDefaultPublicKeyPath(char* outPath, size_t outSize) {
    if (!outPath || outSize < MAX_PATH) return false;
    char moduleDir[MAX_PATH] = {0};
    if (!GetModuleDir(moduleDir, sizeof(moduleDir))) return false;

    // 查找 license_public.blob
    ...
    for (int i = 0; i < 2; i++) {
        if (searchDirs[i][0]) {
            sprintf_s(outPath, outSize, "%s\\%s", searchDirs[i], kDefaultPublicKeyFile);
        } else {
            sprintf_s(outPath, outSize, ".\\%s", kDefaultPublicKeyFile);
        }
        if (FileExistsA2(outPath)) return true;
    }

    // 查找上層目錄
    char* slash = strrchr(moduleDir, '\\');
    if (slash && slash > moduleDir) {
        *slash = '\0';
        sprintf_s(outPath, outSize, "%s\\%s", moduleDir, kDefaultPublicKeyFile);
        if (FileExistsA2(outPath)) return true;
        sprintf_s(outPath, outSize, "%s\\dist\\%s", moduleDir, kDefaultPublicKeyFile);
        if (FileExistsA2(outPath)) return true;
        sprintf_s(outPath, outSize, "%s\\dist\\win32\\%s", moduleDir, kDefaultPublicKeyFile);
        if (FileExistsA2(outPath)) return true;
    }
...
// 導入 RSA 公鑰
static bool ImportRsaPublicKey(const char* path, BCRYPT_KEY_HANDLE* outKey, char* err, size_t errSize) {
    FILE* f = NULL;
    if (fopen_s(&f, path, "rb") != 0 || !f) {
        if (err) sprintf_s(err, errSize, "Cannot open key file: %s", path);
        return false;
    }
    fseek(f, 0, SEEK_END);
    long size = ftell(f);
    ...
    if (fread(blob.data(), 1, size, f) != (size_t)size) {
        fclose(f);
        if (err) sprintf_s(err, errSize, "Cannot read key file");
        return false;
    }
    fclose(f);
    ...
    NTSTATUS status = BCryptOpenAlgorithmProvider(&hAlg, BCRYPT_RSA_ALGORITHM, NULL, 0);
    if (status < 0) {
        if (err) sprintf_s(err, errSize, "BCryptOpenAlgorithmProvider failed");
        return false;
    }
    status = BCryptImportKeyPair(hAlg, NULL, BCRYPT_RSAPUBLIC_BLOB, outKey, blob.data(), (ULONG)blob.size(), 0);
    BCryptCloseAlgorithmProvider(hAlg, 0);
    if (status < 0) {
        if (err) sprintf_s(err, errSize, "BCryptImportKeyPair failed: 0x%08X", (unsigned int)status);
        return false;
    }
    return true;
}

// 離線卡密驗證（一卡一機，支持 JYLIC1 和 JYLIC2）
bool OfflineLicenseVerifySimple(const char* token, OfflineLicenseInfo* outInfo) {
    ...
    // 解析 token
    std::vector<std::string> parts;
    char tokenCopy[8192] = {0};
    strncpy_s(tokenCopy, compact.c_str(), sizeof(tokenCopy) - 1);
    char* ctx = NULL;
    char* p = strtok_s(tokenCopy, "|", &ctx);
    while (p) {
        parts.push_back(p);
        p = strtok_s(NULL, "|", &ctx);
    }

    // JYLIC1 格式: 8個部分，需要 RSA 簽名驗證
    if (parts.size() == 8 && parts[0] == "JYLIC1") {
        // 構造 payload = parts[0]|...|parts[6]
        std::string payload = parts[0];
        for (size_t i = 1; i < 7; i++) {
            payload += "|";
            payload += parts[i];
        ...
        std::vector<BYTE> sig;
        if (!Base64Decode(parts[7], &sig) || sig.empty()) {
            if (outInfo) outInfo->message = "簽名格式錯誤";
            return false;
        }

        // 獲取公鑰路徑
        char pubPath[MAX_PATH] = {0};
        if (!GetDefaultPublicKeyPath(pubPath, sizeof(pubPath)) || !FileExistsA2(pubPath)) {
            if (outInfo) outInfo->message = "缺少 license_public.blob 公鑰檔";
            return false;
        }
        ...
        char err[256] = {0};
        BCRYPT_KEY_HANDLE hKey = NULL;
        if (!ImportRsaPublicKey(pubPath, &hKey, err, sizeof(err))) {
            if (outInfo) outInfo->message = err;
            return false;
        }
        ...
        if (status < 0) {
            if (outInfo) outInfo->message = "簽名驗證失敗";
            return false;
        }

        // 驗證 HWID
        const char* localHwid = GetMachineHWID();
        if (localHwid && localHwid[0] && parts[2] != localHwid) {
            if (outInfo) {
                outInfo->message = "HWID 不匹配";
                outInfo->hwid = parts[2];
                outInfo->license_id = parts[1];
        ...
        // 檢查過期
        ULONGLONG issued = _strtoui64(parts[4].c_str(), NULL, 10);
        ULONGLONG expires = _strtoui64(parts[5].c_str(), NULL, 10);
        ULONGLONG now = (ULONGLONG)time(NULL);
        ...
        if (outInfo) {
            ...
            ULONGLONG remainSec = expires - now;
            outInfo->days_left = (int)((remainSec + 86399ULL) / 86400ULL);
            outInfo->message = "授權驗證成功";
        }
        return true;
    }

    // JYLIC2 格式: 5個部分，簡單 MD5 校驗
    if (parts.size() == 5 && parts[0] == "JYLIC2") {
        // 驗證 MD5 校驗
        char payload[256] = {0};
        sprintf_s(payload, "JYLIC2|%s|%s|%s", parts[1].c_str(), parts[2].c_str(), parts[3].c_str());
        std::string expectedChecksum = MD5Simple(payload, (int)strlen(payload)).substr(0, 8);

        if (parts[4] != expectedChecksum) {
            if (outInfo) outInfo->message = "校驗失敗";
            return false;
        }

        // 驗證 HWID
        const char* localHwid = GetMachineHWID();
        if (localHwid && localHwid[0] && parts[1] != localHwid) {
            if (outInfo) {
                outInfo->message = "HWID 不匹配";
                outInfo->hwid = parts[1];
            }
    ...
    if (outInfo) outInfo->message = "未知的授權格式";
    return false;
}
