...
static bool IsRanWindowTitle(const wchar_t* title) {
    ...
    return _wcsicmp(normalized, L"亂2online") == 0;
}
...
static BOOL CALLBACK FindRanWindowByTitle(HWND hwnd, LPARAM lParam) {
    ...
    if (title[0]) {
        // 只在窗口包含特定關鍵字時記錄
        if (wcsstr(title, L"亂2 online") || wcsstr(title, L"Game")) {
...
static bool IsKnownGameExeNameW(const wchar_t* exeName) {
    ...
    return _wcsicmp(exeName, L"Game.exe") == 0;
}

static bool IsKnownGameExeNameA(const char* exeName) {
    if (!exeName || !exeName[0]) return false;
    return _stricmp(exeName, "Game.exe") == 0;
}
...
static DWORD FindGameProcessIdByKnownNames(wchar_t* matchedExeName, size_t matchedExeNameCount) {
    ...
    for (BOOL ok = Process32FirstW(snap, &pe); ok; ok = Process32NextW(snap, &pe)) {
        if (!IsKnownGameExeNameW(pe.szExeFile)) continue;
...
static bool TryFindGameWindowWin7(GameHandle* gh) {
    gh->hWnd = FindWindowW(NULL, L"亂2 online");
    if (gh->hWnd) {
        LogGame("[Win7] 找到視窗標題: 亂2 online");
        return true;
    }
...
static bool IsGameModule(const wchar_t* modName, const char* fullPath) {
    if (modName && IsKnownGameExeNameW(modName)) {
        return true;
    }
    ...
    if (fullPath) {
        ...
        if (IsKnownGameExeNameA(name)) return true;
    }
...
static bool IsReasonableBase(DWORD base) {
    // 接受以下範圍：
    // - 0x00080000 (DLL 加載位置，常見於遊戲)
    // - 0x00400000 (標準 PE 映像基底)
    // - 0x10000000+ (其他有效 user-mode 地址)
    if (base < 0x00010000) return false;  // 太低
    if (base >= 0x80000000) return false;  // kernel space
    return true;
}
...
DWORD GetGameBaseAddress(GameHandle* gh) {
    if (!gh || !gh->hProcess) {
        OutputDebugStringA("[偵測] GetGameBaseAddress: gh or hProcess is NULL, returning 0\n");
        return 0;
    }

    bool isWin7 = IsWin7System();
    char dbgBuf[256];
    sprintf_s(dbgBuf, "[偵測] GetGameBaseAddress: pid=%u hProcess=%p\n", gh->pid, gh->hProcess);
    ...
    sprintf_s(dbgBuf, "[偵測] CreateToolhelp32Snapshot: snap=%p\n", snap);
    OutputDebugStringA(dbgBuf);
    if (snap == INVALID_HANDLE_VALUE) {
        DWORD err = GetLastError();
        sprintf_s(dbgBuf, "[偵測] CreateToolhelp32Snapshot 失敗 (code=%u)\n", (unsigned int)err);
        OutputDebugStringA(dbgBuf);
        ...
        while (ok) {
            // 調試：打印所有模組名稱
            sprintf_s(dbgBuf, "[偵測] 枚舉模組: %S (base=0x%08X)\n",
                me.szModule, (unsigned int)me.modBaseAddr);
            OutputDebugStringA(dbgBuf);
            LogGame("[偵測] 枚舉模組: %S (base=0x%08X)", me.szModule, (unsigned int)me.modBaseAddr);

            if (IsGameModule(me.szModule, NULL)) {
                DWORD base = (DWORD)me.modBaseAddr;
                sprintf_s(dbgBuf, "[偵測] CreateToolhelp32Snapshot 找到 %S (base=0x%08X)\n",
                ...
                if (!IsReasonableBase(base)) {
                    sprintf_s(dbgBuf, "[偵測] ❌ Base=0x%08X 不合理\n", (unsigned int)base);
                    OutputDebugStringA(dbgBuf);
                ...
                sprintf_s(dbgBuf, "[偵測] ✅ GetGameBaseAddress 成功 = 0x%08X\n", (unsigned int)base);
                OutputDebugStringA(dbgBuf);
        ...
        CloseHandle(snap);
        OutputDebugStringA("[偵測] CreateToolhelp32Snapshot 未找到 Game.exe\n");
    ...
    if (EnumProcessModules(gh->hProcess, hMods, sizeof(hMods), &cbNeeded)) {
        ...
        sprintf_s(dbgBuf, "[偵測] EnumProcessModules 找到 %d 個模組\n", count);
        OutputDebugStringA(dbgBuf);
        ...
        for (int i = 0; i < count; i++) {
            ...
            if (GetModuleFileNameExA(gh->hProcess, hMod, name, sizeof(name))) {
                ...
                sprintf_s(dbgBuf, "  [%d] %s -> 0x%08X\n", i, shortName, (unsigned int)base);
                OutputDebugStringA(dbgBuf);
                LogGame("  [%d] %s -> 0x%08X", i, shortName, (unsigned int)base);

                if (IsGameModule(NULL, name) && IsReasonableBase(base)) {
                    sprintf_s(dbgBuf, "[偵測] ✅ EnumProcessModules 取得遊戲主模組基址 = 0x%08X\n",
                        (unsigned int)base);
                    OutputDebugStringA(dbgBuf);
                ...
                if (IsGameModule(NULL, name) && !IsReasonableBase(base)) {
                    sprintf_s(dbgBuf, "[偵測] ❌ 遊戲主模組 Base=0x%08X 不合理，忽略\n", (unsigned int)base);
                    OutputDebugStringA(dbgBuf);
    ...
    while (VirtualQueryEx(gh->hProcess, (LPCVOID)addr, &mbi, sizeof(mbi)) != 0) {
        if ((mbi.Protect & (PAGE_READONLY | PAGE_READWRITE | PAGE_EXECUTE_READ | PAGE_EXECUTE_READWRITE)) &&
            ...
            if (header[0] == 0x4D && header[1] == 0x5A) {  // "MZ"
                sprintf_s(dbgBuf, "[偵測] 直接記憶體搜尋找到 MZ header @ 0x%08X\n", addr);
                OutputDebugStringA(dbgBuf);
                LogGame("[偵測] 直接記憶體搜尋找到 MZ header @ 0x%08X", addr);
                if (IsReasonableBase(addr)) {
                    sprintf_s(dbgBuf, "[偵測] ✅ 直接記憶體搜尋成功: 遊戲主模組基址 = 0x%08X\n", addr);
                    OutputDebugStringA(dbgBuf);
    ...
    OutputDebugStringA("[偵測] ❌ 無法取得合理的遊戲主模組基址！\n");
...
bool FindGameProcess(GameHandle* gh) {
    ...
    if (!gh->hWnd) {
        LogGame("[偵測] ❌ 找不到遊戲視窗（亂2 online），將透過 Game.exe 搜尋");
    ...
    if (!pid) {
        ...
        if (pid) {
            ...
            LogGame("[偵測] ❌ 找不到 Game.exe 程序");
        }
    }

    if (!pid) {
        printf("[偵測] ❌ 找不到遊戲行程（Game.exe）！\n");
        LogGame("[偵測] ❌ 找不到遊戲行程（Game.exe）！");
        return false;
    }
    gh->pid = pid;
...
// 獲取插件路徑
static bool GetPluginPath(char* outPath, int pathSize) {
    ...
    if (lastSlash) {
        strcpy_s(lastSlash + 1, MAX_PATH - (lastSlash - exePath) - 1, "NetHook.dll");
    }

    if (GetFileAttributesA(exePath) != INVALID_FILE_ATTRIBUTES) {
        strcpy_s(outPath, pathSize, exePath);
        return true;
    }
    ...
    for (int i = 0; i < 5; i++) {
        ...
        if (lastSlash) {
            strcpy_s(lastSlash + 1, MAX_PATH - (lastSlash - fullPath) - 1, paths[i]);
        }

        if (GetFileAttributesA(fullPath) != INVALID_FILE_ATTRIBUTES) {
            strcpy_s(outPath, pathSize, fullPath);
            return true;
        }
    }
...
// 插件驗證主函數
bool TryVerifyPlugin(DWORD pid, char* errorMsg, int errorMsgSize) {
    if (errorMsg && errorMsgSize > 0) errorMsg[0] = '\0';

    if (pid == 0) {
        if (errorMsg) strcpy_s(errorMsg, errorMsgSize, "PID 無效");
        return false;
    }

    // 檢查管理員
    if (!IsRunningAsAdmin()) {
        if (errorMsg) strcpy_s(errorMsg, errorMsgSize, "需要管理員權限");
        return false;
    }
    ...
    if (!hProcess) {
        DWORD err = GetLastError();
        if (errorMsg) sprintf_s(errorMsg, errorMsgSize, "打開進程失敗: %lu", err);
        return false;
    }

    // 獲取插件路徑
    char pluginPath[MAX_PATH] = {0};
    if (!GetPluginPath(pluginPath, sizeof(pluginPath))) {
        if (errorMsg) strcpy_s(errorMsg, errorMsgSize, "找不到 Jy 插件");
        CloseHandle(hProcess);
        return false;
    }

    LogGame("[Plugin] 驗證插件: %s (PID=%lu)", pluginPath, pid);

    // 方法1: 直接使用 LoadLibraryA（簡單方法）
    // 如果目標是 32 位進程，需要 32 位的 JyTrainer

    // 分配記憶體存放路徑
    SIZE_T pathLen = strlen(pluginPath) + 1;
    LPVOID pRemotePath = VirtualAllocEx(hProcess, NULL, pathLen, MEM_COMMIT, PAGE_READWRITE);

    if (!pRemotePath) {
        DWORD err = GetLastError();
        if (errorMsg) sprintf_s(errorMsg, errorMsgSize, "分配記憶體失敗: %lu", err);
        CloseHandle(hProcess);
        return false;
    }

    // 寫入路徑
    if (!WriteProcessMemory(hProcess, pRemotePath, pluginPath, pathLen, NULL)) {
        DWORD err = GetLastError();
        if (errorMsg) sprintf_s(errorMsg, errorMsgSize, "寫入記憶體失敗: %lu", err);
        VirtualFreeEx(hProcess, pRemotePath, 0, MEM_RELEASE);
        CloseHandle(hProcess);
        return false;
    }

    // 獲取 LoadLibraryA 地址（目標進程的 kernel32.dll）
    LPVOID pLoadLibrary = (LPVOID)GetProcAddress(
        GetModuleHandleA("kernel32.dll"), "LoadLibraryA");

    if (!pLoadLibrary) {
        if (errorMsg) strcpy_s(errorMsg, errorMsgSize, "獲取 LoadLibraryA 失敗");
        VirtualFreeEx(hProcess, pRemotePath, 0, MEM_RELEASE);
        CloseHandle(hProcess);
        return false;
    }
    ...
    if (!hThread) {
        DWORD err = GetLastError();
        if (errorMsg) sprintf_s(errorMsg, errorMsgSize, "創建線程失敗: %lu", err);
        VirtualFreeEx(hProcess, pRemotePath, 0, MEM_RELEASE);
        CloseHandle(hProcess);
        return false;
    }
    ...
    // 檢查結果
    if (hModule == 0 || hModule == 0xFFFFFFFF) {
        if (errorMsg) strcpy_s(errorMsg, errorMsgSize, "插件載入失敗（可能被防護阻止）");
        return false;
    }

    LogGame("[Plugin] ✅ 插件驗證成功! Base=0x%lX", hModule);

    if (errorMsg) sprintf_s(errorMsg, errorMsgSize, "驗證成功! Base=0x%lX", hModule);
    return true;
}
