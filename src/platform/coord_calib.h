...
// 所有座標以 0-1024/768 相對值儲存，點擊時自動轉換為螢幕物理座標
...
namespace CoordConv {
    ...
    // 相對座標 (0-1024, 0-768) → 客戶區像素
    inline void RelToClient(int relX, int relY, int* outX, int* outY, int clientW, int clientH) {
        if (outX) *outX = relX * clientW / 1024;
        if (outY) *outY = relY * clientH / 768;
    }

    // 相對座標 (0-1024, 0-768) → 螢幕絕對座標
    inline bool RelToScreen(HWND hWnd, int relX, int relY, int* outScreenX, int* outScreenY) {
        ...
        int cx = relX * clientW / 1024;
        int cy = relY * clientH / 768;
    ...
    inline bool ScreenToRel(HWND hWnd, int screenX, int screenY, int* outRelX, int* outRelY) {
        ...
        if (outRelX) *outRelX = pt.x * 1024 / clientW;
        if (outRelY) *outRelY = pt.y * 768 / clientH;
        return true;
    }
