...
VisionFusion::FindResult VisionFusion::FindPic(const char* imageName,
    ...
    if (DMWrapper::g_dm.IsInited()) {
        s_dmFindCount++;
        DMWrapper::PicResult dmResult = DMWrapper::g_dm.FindPicResult(adjX1, adjY1, adjX2, adjY2,
                                                            imageName, sim);
...
VisionFusion::StateResult VisionFusion::ClassifyState() {
    ...
    result.state = static_cast<int>(vlmResult.state);
    result.confidence = vlmResult.confidence;
    result.from_dm = false;
    result.error = vlmResult.error;
...
bool VisionFusion::IsState(int state, float minConfidence) {
...
bool VisionFusion::Click(FindResult& result) {
    ...
    ScreenshotUniversal::GameCoords gameCoords = ScreenshotUniversal::ScreenToGame(result.x, result.y, s_hWnd);
...
VisionFusion::Stats VisionFusion::GetStats() {
    ...
    stats.multiscale_count = 0;
    stats.multiscale_success_count = 0;
...
void VisionFusion::Diagnostic(HWND hWnd) {
    ...
    if (cap.success) {
        Logf("VisionFusion", "Capture OK: %dx%d", cap.width, cap.height);
    } else {
        Log("VisionFusion", "Capture FAILED");
    }

    Stats stats = GetStats();
    Logf("VisionFusion", "DM: %d/%d, VLM Fallback: %d",
         stats.dm_success_count, stats.dm_find_count, stats.vlm_fallback_count);
...
VisionFusion::StateResult VisionFusion::WaitForState(int targetState,
                                                     int timeoutMs) {
    VisionFusion::StateResult result;
    DWORD startTime = GetTickCount();
