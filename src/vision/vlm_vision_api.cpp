...
namespace {
    ...
    static void DbgLog(const char* tag, const char* msg) {
        if (!s_debug) return;
    ...
    static void DbgLogf(const char* tag, const char* fmt, ...) {
        if (!s_debug) return;
        char buf[512];
        va_list args;
        va_start(args, fmt);
...
bool VLMVision::Init(const VLMConfig& config) {
    ...
    if (s_inited) {
        ...
        DbgLogf("VLM", "Model: %s", config.model_name.c_str());
    } else {
...
VLMVision::VisionResult VLMVision::ClassifyState(const ImageData& image) {
    ...
    for (int retry = 0; retry <= s_config.max_retries; retry++) {
        if (retry > 0) {
            ...
            DbgLogf("VLM", "Retry attempt %d", retry);
        }
    ...
    if (resp.status_code != 200) {
        ...
        DbgLogf("VLM", "API error: %s", result.error.c_str());
        return result;
    }
    ...
    DbgLogf("VLM", "ClassifyState: issue=%s action=%s conf=%.2f latency=%dms",
    ...
    DbgLogf("VLM", "ClassifyState: state=%d, conf=%.2f, latency=%dms",
