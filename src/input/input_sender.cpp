...
// ============================================================
// Trainer 視窗管理（隱藏 trainer 確保輸入送到遊戲）
// ============================================================
static HWND s_trainerHwnd = NULL;
static bool s_trainerWasVisible = true;
static bool s_hiddenByInput = false;  // 是否被輸入系統隱藏

// 外部變數：從 gui_ranbot.cpp 引用
extern bool g_guiVisible;

extern "C" {
    // 供 main.cpp 註冊 trainer 視窗句柄
    __declspec(dllexport) void RegisterTrainerWindow(HWND hWnd) {
        s_trainerHwnd = hWnd;
    }
}

// 發送輸入前隱藏 trainer 視窗（確保 SendInput/SetForegroundWindow 送到遊戲）
static void HideTrainerWindow() {
    if (!s_trainerHwnd || !IsWindow(s_trainerHwnd)) return;

    // ✅ 只有在 GUI 可見且使用者沒有手動隱藏的情況下才隱藏
    if (g_guiVisible) {
        s_trainerWasVisible = IsWindowVisible(s_trainerHwnd) != FALSE;
        if (s_trainerWasVisible) {
            ShowWindow(s_trainerHwnd, SW_HIDE);
            s_hiddenByInput = true;
        }
    }
}

// 發送輸入後恢復 trainer 視窗
static void RestoreTrainerWindow() {
    if (!s_trainerHwnd || !IsWindow(s_trainerHwnd)) return;

    // ✅ 只有在 GUI 可見且之前被輸入系統隱藏的情況下才恢復
    if (g_guiVisible && s_hiddenByInput) {
        ShowWindow(s_trainerHwnd, SW_SHOW);
        // 重新計時等待 UIThread 處理 WndProc 顯示
        Sleep(50);
        s_hiddenByInput = false;
    }
}

// 重置隱藏標誌（當使用者手動隱藏/顯示 GUI 時調用）
extern "C" {
    __declspec(dllexport) void ResetInputWindowHiddenFlag() {
        s_hiddenByInput = false;
    }
}
...
static bool IsKnownGameWindowTitle(HWND hWnd) {
    ...
    int out = 0;
    for (int i = 0; i < len && out < 255; ++i) {
        if (title[i] == L' ' || title[i] == L'\t' || title[i] == L'\r' ||
            title[i] == L'\n') {
            continue;
        }
        normalized[out++] = title[i];
    }
    CharLowerBuffW(normalized, out);
    return wcscmp(normalized, L"亂2online") == 0;
}
...
static bool IsKnownGameWindow(HWND hWnd) {
    ...
    if (!okGame) {
        ...
        if (now - s_lastWarn > 5000) {
            ...
            Logf("輸入", "⚠️ 拒絕輸入：目標視窗不是遊戲 (標題=%s hWnd=%p pid=%u)",
...
static LPARAM MakeKeyLParam(BYTE vk, UINT scan, bool keyUp) {
...
static bool RelativeToClientClamped(HWND hWnd, int rx, int ry, POINT* outPt) {
    ...
    // 1024x768 座標系統
    int cx = rx * clientW / 1024;
    int cy = ry * clientH / 768;
...
// 黑屏修復開關：預設 false，允許 SetForegroundWindow 確保輸入送到遊戲
static std::atomic<bool> s_disableForegroundWindow{false}; // 預設停用（修復技能/喝水不工作）

bool FocusGameWindow(HWND hWnd) {
    if (!hWnd || !IsWindow(hWnd)) return false;
    if (!IsKnownGameWindow(hWnd)) return false;

    // ✅ 隱藏 trainer 視窗，確保輸入送到遊戲
    HideTrainerWindow();
...
static bool ClientToScreenClamped(HWND hWnd, int cx, int cy, POINT* outPt) {
    ...
    int sx = (int)((int64_t)cx * clientW / 1024);
    int sy = (int)((int64_t)cy * clientH / 768);
...
static void SendMouseClick(HWND hWnd, int cx, int cy, DWORD downFlag, DWORD upFlag) {
    ...
    if (!IsKnownGameWindow(hWnd)) {
        static DWORD s_lastFailLog = 0;
        if (GetTickCount() - s_lastFailLog > 3000) {
            Logf("輸入", "❌ SendMouseClick: 視窗不是遊戲進程");
            s_lastFailLog = GetTickCount();
        }
        return;
    }
    ...
    if (!RelativeToClientClamped(hWnd, cx, cy, &pt)) {
        Logf("輸入", "❌ SendMouseClick: RelativeToClientClamped 失敗");
        return;
    }

    UINT downMsg = (downFlag == MOUSEEVENTF_RIGHTDOWN) ? WM_RBUTTONDOWN : WM_LBUTTONDOWN;
    UINT upMsg = (downFlag == MOUSEEVENTF_RIGHTDOWN) ? WM_RBUTTONUP : WM_LBUTTONUP;
    LPARAM lParam = MAKELPARAM((unsigned short)pt.x, (unsigned short)pt.y);

    SendMessageA(hWnd, downMsg, 0, lParam);
    Sleep(10);
    SendMessageA(hWnd, upMsg, 0, lParam);
    Sleep(10);
    Logf("輸入", "📍 [派] 右鍵點擊完成");
}
...
void SendMouseClickDirect(HWND hWnd, int cx, int cy, DWORD downFlag, DWORD upFlag) {
    (void)upFlag;
    if (!hWnd || !IsWindow(hWnd)) {
        Logf("輸入", "❌ SendMouseClickDirect: hWnd 無效 hWnd=%p", hWnd);
        return;
    }
    ...
    POINT pt;
    if (!RelativeToClientClamped(hWnd, cx, cy, &pt)) {
        Logf("輸入", "❌ SendMouseClickDirect: RelativeToClientClamped 失敗");
        return;
    }
    ...
    Logf("輸入", "📍 [WindowMsg] SendMessageA 點擊 client=(%d,%d)", pt.x, pt.y);
    ...
    Logf("輸入", "📍 [WindowMsg] 點擊完成");
}

// ============================================================
// 內部：通用按鍵發送
// ============================================================
static void SendKeyboardInput(BYTE vk, HWND hWnd, DWORD extraFlags) {
    if (!vk) return;
    if (!g_Running || !g_cfg.active.load()) return;

    // ✅ 隱藏 trainer 確保 SendInput 送到遊戲（SendInput 送至前景視窗）
    HideTrainerWindow();

    UINT scan = MapVirtualKeyA(vk, MAPVK_VK_TO_VSC);

    // 先送視窗訊息，再用 SendInput 補一發真實前景按鍵。
    // DirectX 遊戲常會忽略單純 PostMessage，技能/喝水/撿物必須靠 SendInput。
    if (hWnd && IsWindow(hWnd)) {
        LPARAM downLP = MakeKeyLParam(vk, scan, false);
        LPARAM upLP = MakeKeyLParam(vk, scan, true);
...
void RClickFast(HWND hWnd, int cx, int cy) {
    ...
    if (IsWin7Platform()) {
        ...
        if (now - s_lastWin7RClickLog > 2000) {
            ...
            Logf("輸入", "Win7 RClickFast rel=(%d,%d) client=(%d,%d) screen=(%d,%d) clientSize=%ldx%ld",
...
void ClickAttackPoint(HWND hWnd, int pointIndex) {
    // ✅ 全域停止旗標檢查
    if (!g_Running || !g_cfg.active.load()) {
        return;
    }

    if (pointIndex < 0 || pointIndex >= Coords::ATTACK_SCAN_COUNT) {
        Logf("輸入", "❌ ClickAttackPoint: pointIndex=%d 超出範圍", pointIndex);
        return;
    }

    if (!hWnd || !IsWindow(hWnd)) {
        Logf("輸入", "❌ ClickAttackPoint: hWnd=%p 不是有效視窗", hWnd);
        return;
    }

    Coords::ScanPoint actualPt =
        Coords::GetAttackPoint(pointIndex, g_cfg.attackCenter());

    Logf("戰鬥", "圓形點[%d] 座標(%d,%d)",
        pointIndex + 1,
        actualPt.x, actualPt.z);

    POINT clientPt;
    if (!RelativeToClientClamped(hWnd, actualPt.x, actualPt.z, &clientPt)) {
        Logf("輸入", "❌ ClickAttackPoint: RelativeToClientClamped 失敗");
        return;
    }

    POINT screenPt = clientPt;
    if (!ClientToScreen(hWnd, &screenPt)) {
        Logf("輸入", "❌ ClickAttackPoint: ClientToScreen 失敗");
        return;
    }

    FocusGameWindow(hWnd);
    Sleep(IsWin7Platform() ? 35 : 15);
    SetCursorPos(screenPt.x, screenPt.y);

    LPARAM lParam = MAKELPARAM((unsigned short)clientPt.x, (unsigned short)clientPt.y);
    SendMessageA(hWnd, WM_MOUSEMOVE, 0, lParam);
    Sleep(IsWin7Platform() ? 18 : 8);
    ...
    Logf("輸入", "📍 [AttackPoint] 右鍵 screen=(%d,%d) client=(%d,%d)",
...
void SendKeyDirect(HWND hWnd, BYTE vk) {
    ...
    if (!hWnd || !vk) {
        Logf("輸入", "❌ SendKeyDirect: hWnd=%p vk=%d", (void*)hWnd, vk);
        return;
    }
    ...
    Logf("輸入", "📍 [SendKeyDirect] PostMessageA WM_KEYDOWN vk=%d scan=%d", vk, scan);
    ...
    Logf("輸入", "📍 [SendKeyDirect] 按鍵完成 vk=%d", vk);
}
...
void SendKeyInputFocused(BYTE vk, HWND hWnd) {
    ...
    if (!g_Running) {
        ...
        if (s_errCount < 5) {
            UIAddLog("[SendKey] g_Running=false, vk=0x%02X 跳過", vk);
            s_errCount++;
        }
        return;
    }
    // ✅ Bot 暫停時不發送輸入（不隱藏 trainer）
    if (!g_cfg.active.load()) {
        static int s_errCount2 = 0;
        if (s_errCount2 < 5) {
            UIAddLog("[SendKey] g_cfg.active=false, vk=0x%02X 跳過", vk);
            s_errCount2++;
        }
        return;
    }

    if (!hWnd || !vk || !IsWindow(hWnd)) {
        UIAddLog("[SendKey] hWnd=%p vk=%d 參數無效", hWnd, vk);
        return;
    }
    FocusGameWindow(hWnd);
    Sleep(IsWin7Platform() ? 35 : 15);
    SendKeyboardInput(vk, hWnd, 0);
    static DWORD s_lastFocusedLog = 0;
    DWORD now = GetTickCount();
    if (now - s_lastFocusedLog > 5000) {
        Logf("輸入", "[SendInput] focused vk=0x%02X hWnd=%p", (unsigned)vk, hWnd);
        s_lastFocusedLog = now;
    }
}
