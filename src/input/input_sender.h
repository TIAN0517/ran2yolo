...
// Trainer 視窗註冊（main.cpp 初始化時呼叫，確保輸入送到遊戲）
extern "C" __declspec(dllexport) void RegisterTrainerWindow(HWND hWnd);

// 重置視窗隱藏標誌（當使用者手動隱藏/顯示 GUI 時調用）
extern "C" __declspec(dllexport) void ResetInputWindowHiddenFlag();
