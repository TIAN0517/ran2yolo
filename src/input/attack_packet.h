...
// ============================================================
// NPC 對話封包 (0x34D6) - 14 bytes
//
// 格式：[2B size][2B msgId][npcId]
// ============================================================
#pragma pack(push, 1)
struct NpcTalkPacket {
    WORD   wSize;        // 0x0008 (14 bytes)
    WORD   wMsgId;      // 0x34D2 (NPC_TALK) ✅ IDA+CE
    DWORD  dwNpcId;     // NPC ID
    DWORD  dwUnknown;    // 未知字段 (通常為 0)
};
#pragma pack(pop)
static_assert(sizeof(NpcTalkPacket) == 12, "NpcTalkPacket must be 12 bytes");

// ============================================================
// NPC 購買封包 (0x34F7) - 33 bytes
//
// 格式：[2B size][2B msgId][npcName][tab][slot][quantity]
// ============================================================
#pragma pack(push, 1)
struct NpcBuyPacket {
    WORD   wSize;        // 0x0020 (32 bytes)
    WORD   wMsgId;      // 0x34F7 (NPC_BUY)
    char   szNpcName[12]; // NPC 名稱 (12 bytes, null-terminated)
    BYTE   byTab;        // Tab 分頁索引
    BYTE   bySlot;      // 插槽索引
    WORD   wPadding;    // 對齊
    DWORD  dwQuantity;  // 購買數量
    DWORD  dwPrice;     // 價格 (可選，服務器計算)
    DWORD  dwUnknown;   // 未知
};
#pragma pack(pop)
static_assert(sizeof(NpcBuyPacket) == 32, "NpcBuyPacket must be 32 bytes");

// ============================================================
// NPC 出售封包 (0x34F8) - 22 bytes
//
// 格式：[2B size][2B msgId][slotIndex][itemId][quantity]
// ============================================================
#pragma pack(push, 1)
struct NpcSellPacket {
    WORD   wSize;        // 0x0015 (21 bytes)
    WORD   wMsgId;      // 0x34F8 (NPC_SELL)
    WORD   wSlotIndex;  // 背包插槽索引 (0-77)
    DWORD  dwItemId;    // 物品 ID
    DWORD  dwQuantity;  // 出售數量
    DWORD  dwUnknown1;  // 未知
    DWORD  dwUnknown2;  // 未知
};
#pragma pack(pop)
static_assert(sizeof(NpcSellPacket) == 22, "NpcSellPacket must be 22 bytes");
...
// NPC 對話封包
// npcId: NPC 的伺服器 ID
extern bool SendNpcTalkPacket(DWORD npcId);

// NPC 購買封包
// npcName: NPC 名稱 (最多11字符)
// tabIndex: Tab 分頁 (0=第一頁, 1=第二頁, 2=第三頁)
// slotIndex: 插槽索引 (0-7 或更多)
// quantity: 購買數量
extern bool SendNpcBuyPacket(const char* npcName, BYTE tabIndex, BYTE slotIndex, DWORD quantity);

// NPC 出售封包
// slotIndex: 背包插槽索引 (0-77)
// itemId: 物品 ID
// quantity: 出售數量
extern bool SendNpcSellPacket(WORD slotIndex, DWORD itemId, DWORD quantity);
