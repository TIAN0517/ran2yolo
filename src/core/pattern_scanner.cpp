...
namespace {
    ...
    inline void LogScan(const char* fmt, ...) {
        ...
        va_end(args);
        char full[560];
        snprintf(full, sizeof(full), "[PatternScan] %s", buf);
...
ScanResult FindPattern(const Pattern& pattern, DWORD startAddr, DWORD endAddr) {
    ...
    DWORD scanStart = TimeUtil::NowMs();
    ...
    while (currentAddr < endAddr) {
        // 超時檢查
        if (TimeUtil::NowMs() - scanStart > scanTimeout) {
