...
enum class BotState {
    ...
    TOWN_SUPPLY    = 4,  // 城鎮補給（內部使用 s_supplyPhase 0-4）
    BACK_TO_FIELD  = 5,  // 返回野外（按下前點卡）
    PAUSED         = 40, // 轉轉樂安全碼已暫停
...
struct PlayerState {
    ...
    int   arrowCount = 0;   // 箭矢數量 (Game.exe+92F8E0)
    int   talismanCount = 0; // 符咒數量 (Game.exe+92F8E4)
    int   mapId = 0;
    float x     = 0.0f;
    float z     = 0.0f;
    float y     = 0.0f;    // ✅ Y 高度坐標
...
struct BotConfig {
    ...
    std::atomic<int>   potion_interval_ms{100};    // 喝水冷卻間隔（毫秒）
    ...
    std::atomic<bool>  use_packet_attack{false};  // ✅ 使用封包攻擊模式
...
// ── 城鎮補給（完整循環）──
    ...
    // ── 安全碼偵測 ──
    std::atomic<bool>  safe_code_enable{true};         // 啟用安全碼偵測
    ...
    // ═══════════════ 技能系統（1~5攻擊 / 6~0輔助）═══════════════
    // 攻擊技能：數字鍵 1~5（0.8秒輪替）
    // 輔助技能：數字鍵 6~0（GUI設定時間自動施放）

    // ── 圓形掃描設定 ──
    static constexpr int MAX_SCAN_POINTS = 24;        // 3圈 x 8點
    static constexpr int MAX_SKILLS = 10;             // 向後相容
    std::atomic<int> circleRotationIntervalMs{100};   // 圓形選點間隔：8點約0.8秒一圈
    std::atomic<int> attackCenterX{510};              // 攻擊中心點 X
    std::atomic<int> attackCenterZ{380};             // 攻擊中心點 Z

    // 取得攻擊中心點
    Coords::Point attackCenter() const {
        return Coords::Point(attackCenterX.load(), attackCenterZ.load());
    }

    // ── 攻擊技能（1~5 欄位）──
    static constexpr int MAX_ATTACK_SKILLS = 5;
    std::atomic<int> attackSkillCount{5};            // 使用幾個攻擊技能（1~5）
    std::atomic<int> attackSkillInterval{800};       // 攻擊技能間隔 (ms)，預設 0.8秒
    std::atomic<BYTE> attackBarKey{VK_F1};           // 攻擊欄：F1

    // ── 輔助技能（6~0 欄位）──
    static constexpr int MAX_AUX_SKILLS = 5;
    std::atomic<bool> auxEnabled{true};              // 是否啟用輔助技能
    std::atomic<int>  auxSkillCount{5};              // 使用幾個輔助技能（6~0，共5個）
    std::atomic<int>  auxCastIntervalSec{30};        // 輔助技能施放間隔（秒），可在 GUI 設定

    // ── 輪
    std::atomic<int> currentSkillIndex{0};           // 目前輪到哪個攻擊技能（向後相容）
    std::atomic<int> currentAttackSkillIndex{0};     // 目前輪到哪個攻擊技能（0~4）
    std::atomic<int> currentAuxSkillIndex{0};        // 目前輪到哪個輔助技能（0~4）
    std::atomic<DWORD> lastRightClickTime{0};         // 上次右鍵攻擊時間
    std::atomic<DWORD> lastSkillTime{0};             // 上次技能施放時間
    std::atomic<DWORD> lastAuxSkillTime{0};          // 上次輔助技能時間
    std::atomic<DWORD> lastBuffTime{0};               // 上次buff時間（向後相容）
    std::atomic<DWORD> lastCirclePointTime{0};       // 上次圓形選點時間       // 上次圓形選點時間
    // ════════════════════════════════════════════════
...
// 純鍵盤戰鬥模式（不依賴記憶體讀取）
extern void PureCombatInit(HWND hWnd);
extern void PureCombatTick(HWND hWnd);
...
// ═══════════════ GUI 控制函數 ═══════════════
// GUI 內部調整戰鬥意向（不再綁定 F1，避免干擾遊戲技能欄）
extern void CycleCombatIntent();
// GUI 內部調整輔助技能選擇（不再綁定 F2）
extern void CycleSupportSkill(int delta);
// 獲取當前輔助技能索引
extern int GetCurrentSupportSkillIndex();

// ═══════════════ 物資座標設定（持久化）═══════════════
struct SupplyCoords {
    int npcLeftX = 577, npcLeftY = 140;
    int npcRightX = 577, npcRightY = 140;
    int buyBtnX = 440, buyBtnY = 390;
    int sellBtnX = 780, sellBtnY = 300;
    int sellTabX = 0, sellTabY = 0;
    int sellConfirmX = 500, sellConfirmY = 530;
    int consumableTabX = 699, consumableTabY = 253;
    int hpItemX = 320, hpItemY = 280;
    int mpItemX = 360, mpItemY = 280;
    int spItemX = 400, spItemY = 280;
    int qtyInputX = 520, qtyInputY = 420;
    int buyConfirmX = 500, buyConfirmY = 450;
    int petCardX = 800, petCardY = 230;
    int petFoodX = 850, petFoodY = 230;
};

extern bool SaveCoordSettings(const SupplyCoords* cfg);
extern bool LoadCoordSettings(SupplyCoords* cfg);
extern SupplyCoords* GetSupplyCoords();
