...
// ============================================================
// Ran2 Online - 記憶體偏移（CE MCP 驗證）
// 驗證日期：2026-04-30
//
// ✅ 2026-04-30 CE MCP 驗證：
//   GameBase = 0x00080000（Game.exe 載入基底）
//   Name="JyTian" @ base+0x930208 ✓
//   HP=825, MP=73, SP=249, Level=102 @ base+0x930300 ✓
//   STR=35, VIT=222, DEX=91, SPR=100, END=75 @ base+0x930BF4 ✓
//
// ⚠️ 偏移格式說明：
//   所有偏移為 RVA（相對虛擬地址）
//   實際地址 = GameBase + RVA
//   GameBase = GetModuleHandle("Game.exe") 動態獲取
//
// ⚠️ RVA 穩定性：
...
namespace Offsets {
    ...
    // ============================================================
    // Identity（遊戲核心指標）— CE MCP 驗證 2026-04-30
    // ⚠️ EntityPool TLS 依賴，外部無法直接讀取
    // ============================================================
    namespace Identity {
        // ✅ GLCharacter 單例物件（RVA from GameBase）
        constexpr DWORD GLCharacter_Obj     = 0x92F19C;
    ...
    // ============================================================
    // 玩家屬性（✅ CE verified 2026-04-30）
    // ✅ 所有偏移都是靜態 RVA，可直接讀取
    // ✅ 驗證值：HP=825, MaxHP=825, MP=73, MaxMP=73, SP=249, MaxSP=249, Level=102, Gold=1495
    // ⚠️ 注意：GLCHAR 是動態堆對象，沒有靜態指針
    // ⚠️ 座標偏移 (PosX/PosZ/PosY) 需要重新掃描驗證
    // ============================================================
    namespace Player {
        constexpr DWORD Name   = 0x930208;  // ✅ CE 2026-04-30: "JyTian" (Name is part of player data block)
        constexpr DWORD Gold  = 0x930250;  // ✅ CE: 金幣
        constexpr DWORD Level = 0x930248;  // ✅ CE 2026-05-02: 2 Bytes, 值=131
        constexpr DWORD EXP   = 0x9302D8;  // ✅ CE: 當前經驗
        constexpr DWORD EXPMax = 0x9302E0;  // ✅ CE: 升級所需經驗=3455101
        constexpr DWORD HP    = 0x930300;  // ✅ CE 2026-04-30: HP=825
        constexpr DWORD MaxHP = 0x930304;  // ✅ CE 2026-04-30: MaxHP=825
        constexpr DWORD MP    = 0x930308;  // ✅ CE 2026-04-30: MP=73
        constexpr DWORD MaxMP = 0x93030C;  // ✅ CE 2026-04-30: MaxMP=73
        constexpr DWORD SP    = 0x930310;  // ✅ CE 2026-04-30: SP=249
        constexpr DWORD MaxSP = 0x930314;  // ✅ CE 2026-04-30: MaxSP=249
        // ✅ 座標偏移 (2026-04-30 驗證，來自 offsets.ini)
        constexpr DWORD MapID = 0x930DEC;  // ✅ 2026-04-30: MapID=16
        constexpr DWORD PosX  = 0x930DF8;  // ✅ 2026-04-30: X座標
        constexpr DWORD PosZ  = 0x930DFC;  // ✅ 2026-04-30: Z座標 (+4)
        constexpr DWORD PosY  = 0x930E00;  // ✅ 修正: Y座標 (+8, float=4bytes)
        constexpr DWORD CombatPower = 0x932200;  // ✅ CE 2026-04-30: 基礎戰鬥力
        // ✅ 角色屬性 (CE 驗證 2026-05-01)
        // 偏移計算: 實際地址 = GameBase + RVA
        constexpr DWORD STR = 0x932BF4;  // ✅ CE: 力量=35
        constexpr DWORD VIT = 0x932BF8;  // ✅ CE: 體力=478
        constexpr DWORD SPR = 0x932BFC;  // ✅ CE: 精神=91
        constexpr DWORD DEX = 0x932C00;  // ✅ CE: 敏捷=356
        constexpr DWORD END = 0x930C04;  // ✅ CE: 耐力=75
        // ✅ 攻擊力 (CE 驗證 2026-05-01)
        constexpr DWORD PhysAtkMin = 0x933128;  // ✅ CE: 最小攻擊力=612
        constexpr DWORD PhysAtkMax = 0x93312C;  // ✅ CE: 最大攻擊力=628
        constexpr DWORD SprAtkMin = 0x933134;   // ✅ CE: 精神攻擊力=908 (待確認)
        constexpr DWORD SprAtkMax = 0x933138;   // ✅ CE: 精神攻擊力=924 (待確認)
        // ✅ 防禦力
        constexpr DWORD Defense = 0x933130;  // ✅ CE: 防禦力=237
    }
    ...
    namespace Target {
        ...
        // ✅ 2026-05-01 CE 驗證：目標狀態監控
        // 0x932430: 放技能時變1（技能鎖定目標狀態）
        // 0x933E64: 持續監控（目標存在標誌）
        // 0x9329D8: 持續監控
        // 0x9329E0: 持續監控
        // 0x9067CC: 目標存在標誌
        constexpr DWORD SkillLockState = 0x932430;  // ✅ 放技能變1
        constexpr DWORD TargetExist    = 0x933E64;  // ✅ 目標存在標誌
        constexpr DWORD TargetPtr1     = 0x9329D8;  // ✅ 攻擊鎖定（持續=1）
        constexpr DWORD TargetPtr2     = 0x9329E0;  // ✅ 攻擊鎖定（持續=1）
        constexpr DWORD TargetFlag     = 0x9067CC;  // ✅

        // ✅ 2026-05-01 CE 驗證：鎖定怪物信息
        // 0x901070: 當前鎖定怪物的 MID/SID（持續穩定）
        // 0x98105C: 鎖定怪物的 HP（會變化）
        // 0x981050: 怪物類型/狀態
        constexpr DWORD LockedMonsterID   = 0x901070;  // ✅ 怪物ID (85252)
        constexpr DWORD LockedMonsterHP   = 0x98105C;  // ✅ 怪物HP
        constexpr DWORD MonsterType      = 0x981050;  // ✅ 怪物類型
    }

    // ============================================================
    // 背包（✅ CE verified 2026-04-30）
    // ✅ 指標鏈：Game.exe + 0xD30678 → GLCharClient → +0xE5 → 庫存陣列
    // ============================================================
    namespace Inventory {
        // ✅ GLCharClient 指標（固定不變）
        constexpr DWORD GLCharClient_Ptr = 0xD30678;  // GLCharClient 指針

        // ✅ GLCharClient + 0xE5 = 庫存插槽陣列基址
        constexpr DWORD InvArray_Offset = 0xE5;       // 庫存陣列相對於 GLCharClient 的偏移

        // ✅ 庫存插槽結構（每槽 32 bytes）
        // 插槽佈局：
        //   +0x00 = 物品ID (0xFFFFFFFF = 空)
        //   +0x04 = 指針 (指向物品數據結構)
        //   +0x08 = 數量 (DWORD)
        //   +0x0C~+0x1F = 其他數據
        constexpr DWORD ItemId_off      = 0x00;     // 物品ID (FFFFFFFF=空)
        constexpr DWORD ItemPtr_off     = 0x04;     // 物品數據指標
        constexpr DWORD ItemCount_off  = 0x08;     // 物品數量
    ...
    // ============================================================
    namespace QuickSlot {
        constexpr DWORD ArrowCount = 0x9308D8;  // ✅ CE: 箭矢數量=115
        constexpr DWORD TalismanCount = 0x9308DC;  // ✅ CE: 符咒數量=0
    }
    ...
    namespace Packets {
        ...
        // 戰鬥（根據 Wireshark 分析 2026-04-30）
        // ✅ Wireshark 確認：ATTACK MsgID = 0x3A22
        constexpr int ATTACK_MSGID   = 0x3A22;  // 攻擊封包 MsgID
