...
def main():
    print("\n" + "=" * 60)
    print("  RAN2 玩家場景截圖收集腳本")
    print("=" * 60)
    print()
    print("  用法：")
    print("  1. 進入有其他玩家的場景（城鎮、組隊、副本）")
    print("  2. 讓其他玩家在畫面中走動")
    print("  3. 這個腳本會每秒截圖一次")
    print("  4. 按 ESC 鍵停止")
    print()
    ...
    print("\n" + "=" * 50)
    print("  開始截圖！")
    print("  去人多的地方（城鎮、組隊、副本入口）...")
    print("  按 ESC 鍵停止")
    print("=" * 50)

    # 找現有最大編號
    existing = list(OUTPUT_DIR.glob("player_*.jpg"))
    if existing:
    ...
    try:
        while True:
            if windll.user32.GetAsyncKeyState(0x1B) & 0x8000:
                print("\n\n  🛑 停止截圖")
                break

            img = capture_window(hwnd)
            if img is not None:
                filename = f"player_{start_num:06d}.jpg"
                filepath = OUTPUT_DIR / filename

                cv2.imwrite(str(filepath), img, [cv2.IMWRITE_JPEG_QUALITY, 90])

                # 創建空白標籤檔
                label_path = PROJECT_DIR / "training_data" / "labels" / f"player_{start_num:06d}.txt"
                label_path.touch()
