    ...
    int* outBestScore)
{
    ...
    for (int i = 0; i < Coords::ATTACK_SCAN_COUNT; ++i) {
        const int cx = MulDiv(scanPoints[i].x, w, 1024);
        const int cy = MulDiv(scanPoints[i].z, h, 768);
...
static POINT CaptureToGameRelative(int px, int py, int w, int h)
{
    ...
    pt.x = MulDiv(px, 1024, (w > 0 ? w : 1024));
    pt.y = MulDiv(py, 768,  (h > 0 ? h : 768));

    pt.x = ClampInt(pt.x, 0, 1023);
    pt.y = ClampInt(pt.y, 0, 767);
    return pt;
}
